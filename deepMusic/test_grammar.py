...
def __is_scale_tone(chord, note):
    ...
    # D7messscaleType = scale.MajorScale()
...
def __generate_chord_tone(lastChord):
    print("Generating chord tone")
    lastChordNoteNames = [p.nameWithOctave for p in lastChord.pitches]
    print("Generated chord tone: {}", lastChordNoteNames)
...
''' Given the notes in a measure ('measure') and the chords in that measure
    ...
    1) "fullGrammar" : a string that holds the abstract grammar for measure.
     ...
     Symbol  Meaning
        C  Chord tones of the current chord
        L  (chord extensions)
        H —— Either a chord tone
...
def unparse_grammar(m1_grammar, m1_chords):
    ...
    for ix, grammarElement in enumerate(m1_grammar.split(' ')):
        ...
        if (len(terms) == 2): # Case 1: if no < >.
            ...
            print("Terms: {0}, {1}" .format(terms, terms[0]))
            print("lastChord: {}\n\n" .format(lastChord))
            # Case C: chord note.
            if terms[0] == 'C':
                print("Generating")
                insertNote = __generate_chord_tone(lastChord)
                if(insertNote.octave is None):
                    print("No octave!!\n\n\n\n")
                # print("insertNote.octave type: {}" .format(insertNote.type()))
            ...
            if(insertNote.octave is not None):
